...
class MatchDetailsVC: ParentViewController {
        ...
        if page == 0 {
            if fixture.name_first != nil {
                ...
                if "lang".localized == "en"{
                    ...
                    scoreLabel.text = String(fixture.second_goon!) + " - " + String(fixture.first_goon!)
                }
            }
    ...
    func setImageToView(imageView : UIImageView, name : String){
        imageView.sd_setImage(with: URL(string: Urls.baseUrl + name), placeholderImage: UIImage(named: "placeholder"))
    }
    ...
    func getMatchDetails(){
        ...
        presenter.getPlayerMatchDetails(matchLink: link ,onSuccess: { (details, generalData) in
            self.firstClubName.text = generalData.name_first
            self.secondClubName.text = generalData.name_second
            let title = (generalData.name_first ?? "") + " & " + (generalData.name_second ?? "")
            self.pageTitleLbl.text = title
            
            self.setImageToView(imageView: self.firstClubIV, name : generalData.image_first ?? "")
            self.setImageToView(imageView: self.secondClubIV, name : generalData.image_second ?? "")
